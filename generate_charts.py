...
app.get("/health", (req, res) => res.json({ ok: true, engine: "browserless-mapbox-gl-3d", version: "72.85-PREMIUM-ENFORCED" }));
...
function sanitizePremiumTexts(texts, flat) {
  ...
  for (const [key, val] of Object.entries(sanitized)) {
    ...
    // 6. TROPICAL → tropical (in flowing text, not in headers)
    text = text.replace(/\bTROPICAL\b/g, (m, offset) => {
      // Keep uppercase if at start of line or after colon (likely a field label)
      const before = text.substring(Math.max(0, offset - 5), offset);
      if (/[:\n^]/.test(before.trim().slice(-1))) return m;
      return "tropical";
    });
