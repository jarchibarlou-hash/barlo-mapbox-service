...
app.get("/health", (req, res) => res.json({ ok: true, engine: "browserless-mapbox-gl-3d", version: "72.147-stable-3-fixes" }));
...
(function() {
  ...
  map.on('style.load', () => {
    ...
    // v72.147: hide buildings within the parcel polygon via 'within' filter
    try {
      ...
      const buildingLayers = ['3d-buildings', 'building', 'building-top', 'building-outline', 'building-shadow', 'building-3d', 'building-fill', 'building-extrusion'];
      buildingLayers.forEach(layerId => {
        try {
          if (map.getLayer(layerId)) {
            const existing = map.getFilter(layerId);
            const negation = ['!', ['within', PARCEL_POLY_FEATURE]];
            let newFilter;
            if (Array.isArray(existing) && existing[0] === 'all') {
              newFilter = existing.concat([negation]);
            } else if (existing) {
              newFilter = ['all', existing, negation];
            } else {
              newFilter = negation;
            }
    ...
    if (SLIDE4_AI_POLISH_ENABLED && OPENAI_API_KEY) {
      try {
        ...
        if (bestVariation) {
          ...
          const enhancedPath = `hektar/${String(lead_id).trim()}_${slug}/${slide_name}_${UPLOAD_TS}_enhanced.png`;
...
app.post("/generate-massing", async (req, res) => {
  ...
  const enhancedPath = `${folder}/${slideName}_${UPLOAD_TS}_enhanced.png`;
...
ABSOLUTELY NO COOL SHIFT. ABSOLUTELY NO GRAY SHIFT. KEEP EVERYTHING WARM BEIGE.`;
    ...
    return res.json({
      ok: true, cached: false, server_version: "72.147-stable-3-fixes",
...
// ─── START ────────────────────────────────────────────────────────────────────
app.listen(PORT, () => {
  console.log(`BARLO v72.147-stable-3-fixes on port ${PORT}`);
