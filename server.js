...
app.get("/health", (req, res) => res.json({ ok: true, engine: "browserless-mapbox-gl-3d", version: "72.132-MASSING-CLEAN-GROUNDED" }));
...
async function generateMassingHTML(center, zoom, bearing, parcelCoords, envelopeCoords, massingCoords, massingParams, mapboxToken) {
  ...
  // v72.132: re-enable Tilequery intersect filter with 3s race timeout for safety
  const parcelCoordsArray = parcelCoords.map(c => [c.lon, c.lat]);
  const EXCLUDED_IDS = await Promise.race([
    getIntersectingBuildingIds(parcelCoordsArray, mapboxToken),
    new Promise(resolve => setTimeout(() => {
      console.warn('[MASSING v72.132] Tilequery timeout 3s — proceeding without exclusion');
      resolve([]);
    }, 3000))
  ]);
...
(function() {
  ...
  map.on('style.load', () => {
    ...
    // v72.132: GEOMETRIC FILTER using within expression + server-side EXCLUDED_IDS (belt-and-suspenders)
    // Apply to ALL building-related layers discovered at runtime
    const allLayers = map.getStyle().layers.map(l => l.id);
    const buildingLayers = allLayers.filter(id => /build/i.test(id));
    console.log('[MASSING v72.132] building-related layers in style:', buildingLayers.join(', '));

    // v72.132: Parcel polygon feature for within expression
    const parcelFeature = {
      type: 'Feature',
      properties: {},
      geometry: {
      ...
      try {
        if (map.getLayer(layerId)) {
          ...
          if (EXCLUDED_IDS && EXCLUDED_IDS.length > 0) {
            ...
            console.log('[MASSING v72.132] double-filter applied to', layerId, '(within + ID-based for', EXCLUDED_IDS.length, 'buildings)');
          } else {
            // Just within filter if no EXCLUDED_IDS
            combinedFilter = withinClause;
            console.log('[MASSING v72.132] within-filter applied to', layerId);
          }
          map.setFilter(layerId, combinedFilter);
        }
      } catch (e) { console.warn('[MASSING v72.132]', layerId, 'filter error:', e.message); }
    });

    console.log('[MASSING v72.132] Buildings filtered via geometric within + Tilequery intersection');
...
// ─── START ─────────────────────────────────────────
app.listen(PORT, () => {
  console.log(`BARLO v72.132-MASSING-CLEAN-GROUNDED on port ${PORT}`);
