...
app.post("/compute-scenarios", (req, res) => {
  ...
  const flat = {
    ...
    A_role: (sA.role || p.scenario_A_role || "ÉQUILIBRE"),
    B_role: (sB.role || p.scenario_B_role || "OPTIMISATION"),
    C_role: (sC.role || p.scenario_C_role || "INTENSIFICATION"),
...
app.post("/generate-massing", async (req, res) => {
  ...
  try {
    ...
    // ── v72.0-POLISH: AI polish réactivé pour massing ──────────────────────
    // Pattern identique à slide 4 : envoi image clean → OpenAI polish → redraw overlays
    let polishApplied = false;
    if (OPENAI_API_KEY) {
      try {
        console.log("[MASSING-POLISH] Starting AI polish v72.0...");
        const b64Input = pngClean.toString("base64");
        console.log(`[MASSING-POLISH] Input: ${pngClean.length} bytes, b64: ${b64Input.length} chars`);

        const massingPolishPrompt = `Transform this 3D architectural massing diagram into a photo-realistic architectural rendering.

GEOMETRY RULES (ABSOLUTE — ZERO TOLERANCE):
- Keep ALL building footprints, positions, shapes, heights, and sizes EXACTLY as shown. Do NOT move, resize, reshape, add, or remove ANY building volume.
- Keep ALL roads in their exact positions and widths.
- Keep the same camera angle, framing, pitch, and composition. Do NOT crop, rotate, or shift the view.
- Keep the colored accent on the main building (the highlighted massing) EXACTLY as shown — same color, same position.
- Do NOT add text, watermarks, inset images, labels, or UI elements.
- Do NOT add or remove any floors/levels from any building.

VISUAL ENHANCEMENT (MANDATORY):
- TREES: Add small realistic round-canopy trees (dark green, 3D spherical crowns) scattered in green areas around buildings. Canopy diameter 2-4m max, never bigger than a car. About 15-25 trees visible. No trees on top of buildings.
- SHADOWS: Add realistic cast shadows from ALL buildings and trees. Sun from the upper-left. Soft penumbra.
- CONTEXT BUILDINGS: Apply light concrete/gray texture (#c8c4bc) to surrounding buildings. Slightly weathered, realistic.
- MAIN BUILDING: Keep its accent color exactly as shown. Apply a subtle material texture (glass, concrete, or painted) while preserving the color.
- ROADS: Dark asphalt gray (#3a3a3a), slightly textured.
- GRASS/GROUND: Rich green lawn texture with subtle shade variations where green areas exist.
- LIGHTING: Warm afternoon daylight. Natural ambient occlusion at building bases.

QUALITY: Photo-realistic architectural visualization, clean, professional, suitable for a private client presentation.`;
        ...
        console.log(`[MASSING-POLISH] API status: ${oaiRes.status} (${Date.now() - t0}ms)`);
        const oaiJson = await oaiRes.json();
        ...
        if (oaiJson.error) {
          ...
          if (polishedB64) {
            ...
            // Redessiner les overlays massing APRÈS le polish (légende, boussole, annotations)
            ...
            if (!ue2) {
              ...
              enhancedUrl = pd2.publicUrl + `?v=${Date.now()}`;
              polishApplied = true;
              console.log(`✓ [MASSING-POLISH] Enhanced uploaded: ${enhancedUrl} (${Date.now() - t0}ms)`);
            } else {
              console.warn("[MASSING-POLISH] Upload error:", ue2.message);
            }
          } else {
      ...
      console.log("[MASSING-POLISH] OPENAI_API_KEY absent — skipping");
    }
    // ── Fin AI polish ─────────────────────────────────────────────────────────
    console.log(`[MASSING] v72.0-POLISH complete — polish_applied: ${polishApplied} (${Date.now() - t0}ms)`);
    return res.json({
      ok: true, cached: false, server_version: "72.0-POLISH",
...
// ─── START ────────────────────────────────────────────────────────────────────
app.listen(PORT, () => {
  console.log(`BARLO v72.0-POLISH on port ${PORT}`);
