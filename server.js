...
function computeZoomMassing(coords, cLat, cLon) {
  ...
  const mpp = (ext * 0.9) / 1280;  // v51.1: zoom x2 — cadrage serré sur la parcelle
  const z = Math.log2(156543.03 * Math.cos(cLat * Math.PI / 180) / mpp);
...
function generateMapHTML(center, zoom, bearing, parcelCoords, envelopeCoords, mapboxToken) {
  ...
  // v49: auto-detect accès principal (milieu du côté face route = plus haute latitude)
  let bestMidLat = -Infinity, bestI = 0;
  for (let i = 0; i < parcelCoords.length; i++) {
    const j = (i + 1) % parcelCoords.length;
    const midLat = (parcelCoords[i].lat + parcelCoords[j].lat) / 2;
    if (midLat > bestMidLat) { bestMidLat = midLat; bestI = i; }
  }
  const accJ = (bestI + 1) % parcelCoords.length;
  const accMidLat = (parcelCoords[bestI].lat + parcelCoords[accJ].lat) / 2;
  const accMidLon = (parcelCoords[bestI].lon + parcelCoords[accJ].lon) / 2;
  const edgeDx = parcelCoords[accJ].lon - parcelCoords[bestI].lon;
  ...
  const accessLineGeoJSON = { type: "Feature", properties: {},
    geometry: { type: "LineString", coordinates: [[accMidLon, accMidLat], [accEndLon, accEndLat]] } };
  const accessPointGeoJSON = { type: "Feature", properties: { label: "Accès" },
...
(function() {
  ...
  // v49 HEKTAR PRO — vert gazon + routes beige + labels rues
  window._map = null; // v50: will be set after map creation for pixel projection
  const hektarStyle = {
    "version": 8, "name": "Hektar Pro",
    ...
    "layers": [
      ...
      { "id": "water", "type": "fill", "source": "composite", "source-layer": "water", "paint": { "fill-color": "#a8cce0" } },
...
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  body { width: 1280px; height: 1280px; overflow: hidden; background: #f5f3ef; }
  #map { width: 1280px; height: 1280px; }
  .mapboxgl-ctrl-logo, .mapboxgl-ctrl-attrib, .mapboxgl-ctrl-group { display: none !important; }
</style>
<script src="https://api.mapbox.com/mapbox-gl-js/v2.15.0/mapbox-gl.js"></script>
...
(function() {
  ...
  const hektarStyle = {
    ...
    "layers": [
      ...
      { "id": "road-label-major", "type": "symbol", "source": "composite", "source-layer": "road",
        "filter": ["match", ["get", "class"], ["secondary", "tertiary", "primary", "trunk", "motorway"], true, false],
        "layout": { "text-field": ["coalesce", ["get", "name_fr"], ["get", "name"]], "text-font": ["DIN Pro Medium", "Arial Unicode MS Regular"],
          "text-size": ["interpolate", ["linear"], ["zoom"], 14, 8, 17, 11], "symbol-placement": "line", "text-max-angle": 35, "text-padding": 10 },
        "paint": { "text-color": "#aaa59a", "text-halo-color": "rgba(255,255,255,0.9)", "text-halo-width": 1.5 } }
    ]
  };
  const map = new mapboxgl.Map({
    container: 'map', style: hektarStyle,
  ...
  map.on('style.load', () => {
    // v51.1: éclairage doux pour ombres portées visibles
    map.setLight({ anchor: 'map', color: '#ffffff', intensity: 0.6, position: [1.2, 210, 30] });

    // v51.1: bâtiments environnants LÉGERS — gris très clair, semi-transparents, pas de contours noirs
    map.addLayer({
      id: '3d-buildings', source: 'composite', 'source-layer': 'building',
      filter: ['==', 'extrude', 'true'], type: 'fill-extrusion', minzoom: 13,
      paint: {
        'fill-extrusion-color': '#e8e6e2',
        'fill-extrusion-height': ['match', ['%', ['to-number', ['id']], 50],
              0,3.5, 1,3.5, 2,3.5, 3,3.5, 4,3.5, 5,3.5, 6,3.5, 7,3.5, 8,3.5, 9,3.5,
              10,3.5, 11,3.5, 12,3.5, 13,3.5, 14,3.5, 15,3.5, 16,3.5, 17,3.5, 18,3.5, 19,3.5,
              ...
              7],
        'fill-extrusion-base': 0,
        'fill-extrusion-opacity': 0.55, 'fill-extrusion-vertical-gradient': true,
      },
    });
  ...
  map.on('idle', () => {
    ...
    // v51.1: arbres Hektar — subtils, vert doux
    map.addSource('trees', { type: 'geojson', data: { type: 'FeatureCollection', features: treeFeatures } });
    map.addLayer({ id: 'tree-shadow', type: 'circle', source: 'trees',
      paint: { 'circle-radius': ['*', ['get', 'radius'], 0.9], 'circle-color': '#8aaa70', 'circle-opacity': 0.15,
        'circle-translate': [3, 3], 'circle-blur': 0.6 } });
    map.addLayer({ id: 'tree-canopy', type: 'circle', source: 'trees',
      paint: { 'circle-radius': ['get', 'radius'], 'circle-color': '#a0c878',
        'circle-stroke-width': 0.5, 'circle-stroke-color': '#80a858', 'circle-opacity': 0.55, 'circle-blur': 0.25 } });
    setTimeout(() => { window.__MAP_READY = true; }, 1200);
  });
  setTimeout(() => { window.__MAP_READY = true; }, 30000);
})();
...
function drawOverlays(ctx, W, H, BH, p) {
  ...
  legItems.forEach((item, i) => {
    ...
    if (item.type === "rect") {
      ...
      ctx.beginPath(); ctx.arc(36, iy + 6, 5, 0, 2 * Math.PI);
...
function drawGeometryOverlay(ctx, W, H, projectedGeometry, roadLabels) {
  ...
  if (roadLabels && roadLabels.length > 0) {
    roadLabels.forEach(lbl => {
      ...
      let angle = lbl.angle || 0;
      if (angle > Math.PI / 2) angle -= Math.PI;
      if (angle < -Math.PI / 2) angle += Math.PI;
      ctx.rotate(angle);
      ctx.font = lbl.isMajor ? "bold 11px Arial" : "10px Arial";
      ctx.textAlign = "center";
      ctx.textBaseline = "middle";
      ...
      ctx.lineWidth = 3;
      ctx.lineJoin = "round";
      ctx.strokeText(lbl.name, 0, 0);
      ctx.fillStyle = lbl.isMajor ? "#5a4a2a" : "#7a6a4a";
      ctx.fillText(lbl.name, 0, 0);
      ctx.restore();
    });
  }
}
...
app.post("/generate", async (req, res) => {
  const t0 = Date.now();
  console.log("═══ /generate v51.1 (composite-fidelity) ═══");
  const {
    ...
    style_ref_url = null,
    enhance = true,
    skip_generate = false,  // v51.0: si true, skip la génération (pour ne pas bloquer le massing)
  } = req.body;
  // v51.0: skip rapide si demandé
  if (skip_generate === true || skip_generate === "true") {
    console.log("SKIP /generate (skip_generate=true)");
    return res.json({ ok: true, skipped: true, message: "Slide 4 generation skipped" });
  }
  ...
  // v50: compute access arrow coords (same logic as generateMapHTML) for post-OpenAI overlay
  let bestMidLat = -Infinity, bestI = 0;
  ...
  const accJ = (bestI + 1) % coords.length;
  const accMidLat = (coords[bestI].lat + coords[accJ].lat) / 2;
  const accMidLon = (coords[bestI].lon + coords[accJ].lon) / 2;
  const edgeDx = coords[accJ].lon - coords[bestI].lon;
  ...
  try {
    ...
    const projectionResult = await page.evaluate((parcelPts, envelopePts, accessLine, accessPt) => {
      const m = window._map;
      if (!m || !m.project) return null;
      try {
        const projectPts = (pts) => pts.map(p => {
          const px = m.project([p.lon, p.lat]);
          return { x: Math.round(px.x * 10) / 10, y: Math.round(px.y * 10) / 10 };
        });
        // Extract visible road labels from Mapbox rendered features
        const roadLabels = [];
        try {
          const labelLayers = ['road-label-major', 'road-label-street'];
          labelLayers.forEach(layerId => {
            const features = m.queryRenderedFeatures(undefined, { layers: [layerId] });
            if (features) features.forEach(f => {
              if (f.properties && (f.properties.name_fr || f.properties.name)) {
                const name = f.properties.name_fr || f.properties.name;
                // Get center point of the feature for label placement
                const geom = f.geometry;
                if (geom && geom.type === 'LineString' && geom.coordinates.length >= 2) {
                  const mid = Math.floor(geom.coordinates.length / 2);
                  const coord = geom.coordinates[mid];
                  const px = m.project(coord);
                  // Get bearing of road segment for text rotation
                  const prev = geom.coordinates[Math.max(0, mid - 1)];
                  const next = geom.coordinates[Math.min(geom.coordinates.length - 1, mid + 1)];
                  const pxPrev = m.project(prev);
                  const pxNext = m.project(next);
                  const angle = Math.atan2(pxNext.y - pxPrev.y, pxNext.x - pxPrev.x);
                  if (!roadLabels.find(r => r.name === name && Math.abs(r.x - px.x) < 50)) {
                    roadLabels.push({
                      name, x: Math.round(px.x), y: Math.round(px.y),
                      angle, isMajor: layerId === 'road-label-major'
                    });
                  }
                }
              }
        ...
        } catch(e) { /* road labels extraction optional */ }
        return {
          geometry: {
      ...
      envelopeCoords.map(c => ({ lat: c.lat, lon: c.lon })),
      [[accMidLon, accMidLat], [accEndLon, accEndLat]],
      [accEndLon, accEndLat]
    );
    const projectedGeometry = projectionResult ? projectionResult.geometry : null;
    const roadLabels = projectionResult ? projectionResult.roadLabels || [] : [];
    console.log(`Projected geometry: ${projectedGeometry ? 'OK' : 'FALLBACK'}, road labels: ${roadLabels.length} (${Date.now() - t0}ms)`);
    ...
    const shouldEnhance = enhance !== false && enhance !== "false" && enhance !== 0;
    if (OPENAI_API_KEY && shouldEnhance) {
      try {
