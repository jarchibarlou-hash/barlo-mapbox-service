...
function buildTemplateTexts(flat, scenarios) {
  ...
  function buildFinancial(sc) {
    ...
    const units = f(`${sc}_units`);
    // v74.12 — ton démonstratif, calculs en bold, structure punchy
    return `**${costM2}/m² de SDP** pour un standing ${standing} à ${city}, en cohérence avec le marché local.\n\n**Calcul** : ${sdp} m² × ${costM2}/m² ≈ **${costTotal}**\n\nVentilation prévisionnelle (cf. graphique ci-après) :\n- Gros œuvre : **${ventGoPct}** (${ventGo} FCFA) — fondations, structure béton, dalles\n- Second œuvre : **${ventSoPct}** (${ventSo} FCFA) — menuiseries, finitions, étanchéité\n- Lots techniques : **${ventLtPct}** (${ventLt} FCFA) — électricité, plomberie, CVC\n- VRD : **${ventVrdPct}** (${ventVrd} FCFA) — réseaux, voirie, assainissement\n\nFourchette budgétaire globale : ${fourchette}.\nHonoraires maîtrise d'œuvre : **${honoBas}M à ${honoHaut}M FCFA** (${honoTauxBas} à ${honoTauxHaut} des travaux).\n\n**Position vs budget client (${budgetFcfa})** : ${budgetFit}.\n**Coût par unité** : ${costUnit}/unité (${costTotal} ÷ ${units} unités).\n\nEstimations à affiner en phase APS/APD sur la base d'un chiffrage détaillé par lots.`;
  }
  ...
  function buildRisk(sc) {
    ...
    // v74.12 — score en titre, sections numérotées en bold, prose ressérée
    return `**Score global ${score}/100** — profil ${profil}.\n\nLe radar ci-contre détaille les 7 critères pondérés. Points clés :\n\n**1. Urbanisme — COS et conformité**\nCOS utilisé : **${cosPct} %** du maximum autorisé (${cosRegl}), soit ${sdp} m² SDP sur ${sdpMax} théoriques. Hauteurs et retraits conformes.\n\n**2. Compacité — surface habitable**\n${compaciteAnalysis}\n\n**3. Budget**\n${budgetAnalysis}\n\n**4. Stationnement**\n${parking}.\n\n**5. Constructibilité**\nR+${levels} en standing ${standing} : complexité ${risqueCompacite === "élevé" ? "significative" : risqueCompacite === "modéré" ? "modérée" : "standard"} pour les entreprises locales. Système poteau-poutre béton armé maîtrisé ; rigueur requise sur la qualité du béton et le ferraillage à ${levels} niveaux.\n\nSynthèse visuelle ci-dessous : radar de profil, jauge de score, barres des facteurs de risque.`;
  }
  ...
  // v74.12 — intro punchy, ton démonstratif, données clés en bold
  texts.slide_3_intro_text = `**${f("site_area")} m²** à ${f("city")}, programme **${f("program_main")}** pour **${f("A_units")} unités** en standing ${f("standing_level").toLowerCase()}, budget de référence **${f("budget_fcfa")}**.\n\nLe site est encadré par un **COS de ${f("site_cos_regl")}**, un **CES de ${f("site_ces_regl")} %**, et des retraits qui réduisent significativement l'emprise constructible.\n\nTrois scénarios ont été chiffrés pour identifier la configuration optimale :\n- **Scénario A (${f("A_role")})** — maximisation de la densité\n- **Scénario B (${f("B_role")})** — compromis densité / coût\n- **Scénario C (${f("C_role")})** — approche prudente, coût maîtrisé\n\nCe rapport analyse chaque scénario sous l'angle volumétrique, financier et réglementaire, et oriente vers le scénario le plus pertinent au regard du budget et des priorités du porteur de projet.\n\n**Données clés du projet :**\n- Terrain : **${f("site_area")} m²** | Emprise constructible (après retraits) : **${f("retrait_emprise_constructible")}**\n- Enveloppe : **${f("envelope_w")} × ${f("envelope_d")} m**\n- SDP max théorique (COS) : **${f("site_sdp_max")}**\n- Budget : **${f("budget_fcfa")}** | Standing : ${f("standing_level").toLowerCase()}\n- Zone climatique : ${(f("orient_zone") || "tropical").toLowerCase()} | Mitoyenneté : **${f("retrait_mitoyennete")}** côtés`;
  texts.slide_3_programme_text = "";
  // ── SLIDE 4: Terrain ──
  // v74.12 — site analysis : faits, retraits, impact net démontré
  texts.slide_4_text = `Terrain de **${f("site_area")} m²** dans un tissu urbain dense de ${f("city")}. Géométrie régulière **${f("envelope_w")} × ${f("envelope_d")} m**, soit **${f("envelope_area")} m²** brut.\n\n**Retraits réglementaires :**\n- Recul avant : **${f("retrait_avant")}** (par rapport à la voie)\n- Recul latéral : **${f("retrait_lateral")}** de chaque côté\n- Recul arrière : **${f("retrait_arriere")}**\n- Mitoyenneté : **${f("retrait_mitoyennete")}** côtés\n\n**Impact net** : l'emprise constructible chute de **${f("retrait_reduction_pct")}** et tombe à **${f("retrait_emprise_constructible")}** exploitables.\n\nLe programme cible **${f("A_units")} unités** en usage ${f("program_main")}, standing ${f("standing_level").toLowerCase()}.\n\n- Zone climatique : ${f("orient_zone").toLowerCase()}\n- COS réglementaire : **${f("site_cos_regl")}**\n- CES réglementaire : **${f("site_ces_regl")} %**`;
  ...
  // v74.12 — conclusion punchy avec verdict en bold et appel à l'action
  texts.conclusion_summary_text = `Diagnostic d'un terrain de **${f("site_area")} m²** à ${f("city")} sous trois scénarios — ${f("A_role")} (A), ${f("B_role")} (B), ${f("C_role")} (C) — pour un programme **${f("A_units")} unités** en ${f("program_main")}, standing ${f("standing_level").toLowerCase()}.\n\n**Verdict : Scénario ${rec} recommandé (${f("rec_score")}/100)** — voir fiche récapitulative ci-dessous. Il offrirait le meilleur compromis entre :\n- **Respect du programme** : ${f("A_units")} unités livrées en R+${f("rec_levels")}\n- **Proximité budget** : **${f("rec_cost_total")}** vs **${f("budget_fcfa")}** visés\n- **Conformité urbanistique** : COS à ${f("rec_cos_pct")} % du maximum autorisé\n- **Simplicité de mise en œuvre** : R+${f("rec_levels")} maîtrisable par les entreprises locales\n\n**Prochaine étape** : engager les études APS/APD et l'étude géotechnique pour confirmer ces hypothèses et affiner le chiffrage.`;
