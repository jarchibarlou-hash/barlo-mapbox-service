...
app.get("/health", (req, res) => res.json({ ok: true, engine: "browserless-mapbox-gl-3d", version: "41.0-roads-mandatory" }));
...
app.post("/generate", async (req, res) => {
  const t0 = Date.now();
  console.log("═══ /generate v41 (routes MANDATORY) ═══");
  ...
  try {
    ...
    if (OPENAI_API_KEY) {
      try {
        ...
        form.append("prompt", "Restyle this axonometric urban planning map into a premium architectural site analysis illustration.\n\nGEOMETRY - ABSOLUTE CONSTRAINTS:\n- Keep EXACTLY the same camera angle, pitch, bearing and composition\n- Keep EXACTLY the same building footprints, positions and heights\n- Keep EXACTLY the same road network\n- Keep EXACTLY the red parcel and dashed red envelope at their exact positions\n- Do NOT move, add or remove any building or road\n\nBUILDINGS - CRITICAL:\n- Building rooftops: BRIGHT PURE WHITE #ffffff, stark and clean\n- Building sunlit vertical faces: PURE WHITE #ffffff to very light warm white #faf9f6\n- Building shadow vertical faces: warm gray #9a9690\n- Building EDGES AND OUTLINES: MANDATORY strong black lines #1a1a1a on ALL edges, corners and roof lines - this is the most important visual element - make them bold, crisp, architectural - like a physical scale model with inked edges\n- Cast shadows: solid warm gray #c4c0b8, sharp and directional\n- The contrast between pure white buildings and black edges is THE defining visual quality\n\nGROUND AND VEGETATION - CRITICAL:\n- Ground between buildings: fresh vivid green #7ab83a slightly softened with warm overlay - vibrant but not neon, natural outdoor feel\n- Grass texture: visible fine grain, slightly muted green #6aa030 with subtle warm tone, like sunlit grass\n- Trees: round canopy top-view, rich dark green #3d7a1a with highlight #5aaa28, slightly translucent canopy edges\n- Tree shadows on ground: dark green #2d5a12\n- The green must be VIVID and clearly green, but with a slight warm softness - NOT fluorescent, NOT artificial\n- Place trees densely: along every sidewalk, in every open space, in all courtyards\n- At least 30-40 trees visible, covering most open ground areas\n- Parks and open areas: filled with rich green grass texture #7ab83a and dense tree canopies\n- The ground should be predominantly GREEN, not cream or beige\n\nROADS:\n- Road surface: warm light beige #e8e0cc\n- Road borders: slightly darker #d4c9b0\n- Sidewalks: cream #f0ece0\n\nPARCEL:\n- Red fill: semi-transparent rose/pink, flat on ground, under buildings\n- Red outline #d02818: clearly visible\n- Dashed red envelope #d02818: clearly visible\n\nROADS - ABSOLUTELY MANDATORY - DO NOT IGNORE:\n- Roads MUST be clearly wider than in the input - they are the primary urban structure\n- Road surface: warm sandy beige #d4c49a with visible asphalt grain texture, NOT green, NOT grass\n- Road borders/curbs: darker #b8a478, sharp line separating road from sidewalk\n- Sidewalks: narrow cream strip #ede4cc between road edge and grass\n- Main roads: significantly wider, with center dashed line marking\n- Secondary roads: clearly visible, medium width\n- ALL roads must be immediately visible as ROADS - sandy/beige color, textured surface\n- Roads NEVER blend into green areas - the color contrast must be strong\n- The road GRID is the skeleton of the image - make it prominent and legible\n\nBLOCK STRUCTURE - ABSOLUTELY MANDATORY - DO NOT IGNORE:\n- Each city block is an ISLAND of green + buildings, COMPLETELY SURROUNDED by roads\n- Road on ALL 4 sides of every block - no exceptions\n- Green grass stays STRICTLY INSIDE each block boundary\n- Green NEVER crosses over roads into adjacent blocks\n- The alternating pattern ROAD / GREEN-BLOCK / ROAD must be crystal clear\n- Block boundaries must be sharp - road edge is a hard line\n- Urban grid must be immediately readable: blocks are clearly separated islands\n\nSITE PARCEL - CRITICAL POSITION:\n- The red parcel outline marks an EXACT position on the ground - do NOT move it, do NOT shift it, do NOT resize it\n- The parcel must stay EXACTLY where it is in the original image - same block, same position relative to roads and buildings\n- The parcel does NOT overlap roads - if it appears to touch a road, keep it strictly inside the block\n- Inside parcel: bare sandy ground #e8d4b8, no vegetation, flat - reads as empty construction plot\n- The dashed red envelope line INSIDE the parcel must also stay at its exact position\n- Envelope fill: semi-transparent red/orange #d02818 at 15% opacity covering the envelope area\n\nNo text, no labels, no annotations.\nOutput: premium urban planning illustration, clear block structure, vivid green blocks separated by beige textured roads, empty sandy parcel.");
...
app.listen(PORT, () => {
  console.log(`BARLO v41 on port ${PORT}`);
