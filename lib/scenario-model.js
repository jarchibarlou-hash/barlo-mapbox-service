...
(function (root, factory) {
  ...
  function normalizeSuggestion(letter, sc, ctx) {
    ...
    // COS (vocabulaire de Jeremy) = occupation au sol : emprise au sol du bâti / terrain
    const ground = Number(sc.emprise_sol_m2) || fp;
    ...
    return {
      ...
      cos: pv(siteArea > 0 && ground > 0 ? round(ground / siteArea, 3) : null, siteArea > 0 ? SOURCE.BARLO_SUGGESTION : SOURCE.UNKNOWN, "occupation au sol : emprise au sol / surface terrain"),
      cost_per_m2: cost,
      // Réserve pour imprévus gardée sur le budget (B 10 %, C 20 %) et phase 2 de C
      reserve_pct: sc.sdp_limits_v12 && sc.sdp_limits_v12.reserve_pct != null ? pv(sc.sdp_limits_v12.reserve_pct, SOURCE.HYPOTHESIS) : null,
      phase_2: sc.phase_2_v12 || null,
