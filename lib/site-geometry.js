...
(function (root, factory) {
  ...
  function scenarioActual(units, ctx) {
    ...
    const outsideBySide = list.map(() => ({}));  // débord par côté de parcelle (côté dont le retrait n'est pas tenu)
    const parcel = site ? site.parcel : null;
    const overlap = {};
    if (list.length) {
      ...
      for (let i = 0; i < nx; i++) for (let j = 0; j < ny; j++) {
        ...
        ground += cell;
        if (buildable && !pointInPolygon(pt, buildable)) {
          // côtés dont le retrait n'est pas respecté en ce point
          const sides = [];
          if (parcel) for (let e = 0; e < parcel.length; e++) {
            if (distToSegment(pt, parcel[e], parcel[(e + 1) % parcel.length]) < (site.retraits[e] || 0) - 1e-9) sides.push(e);
          }
          for (const k of hits) {
            outside[k] += cell;
            for (const e of sides) outsideBySide[k][e] = (outsideBySide[k][e] || 0) + cell;
          }
        }
    ...
    const checks = [];
    // Nom d'un côté de parcelle comme dans le cockpit : sommets A, B, C… (côté i = sommet i → sommet i+1)
    const letter = i => i < 26 ? String.fromCharCode(65 + i) : String.fromCharCode(65 + Math.floor(i / 26) - 1) + String.fromCharCode(65 + (i % 26));
    const sideName = e => `${letter(e)}–${letter((e + 1) % parcel.length)}`;
    const sidesOf = k => Object.keys(outsideBySide[k]).map(Number).filter(e => outsideBySide[k][e] >= 0.5)
      .sort((a, b) => outsideBySide[k][b] - outsideBySide[k][a]);
    list.forEach((u, k) => {
      if (outside[k] < 0.5) return;
      const sides = parcel ? sidesOf(k) : [];
      const where = sides.length ? ` côté ${sides.map(e => `${sideName(e)} (retrait ${site.retraits[e]} m)`).join(", ")}` : "";
      checks.push({ code: "HORS_ZONE_CONSTRUCTIBLE", level: "error", unit: u.name, m2: Math.round(outside[k]), sides: sides.map(sideName),
        message: `« ${u.name} » dépasse la zone constructible de ${Math.round(outside[k])} m²${where} : ajuste la forme, ou indique ce côté en rue / mitoyen dans le tableau des retraits` });
    });
    ...
    for (const o of overlaps) checks.push({ code: "SUPERPOSITION", level: "error", unit: o.a, m2: o.m2,
      message: `« ${o.a} » et « ${o.b} » se superposent sur ${o.m2} m² au même niveau : si l'un est à l'étage, règle son « étage de départ » (⚙ de l'unité)` });
