  ...
  function scenarioActual(units, ctx) {
    ...
    list.forEach((u, k) => {
      ...
      checks.push({ code: "HORS_ZONE_CONSTRUCTIBLE", level: "error", unit: u.name, m2: Math.round(outside[k]), sides: sides.map(sideName),
        message: `« ${u.name} » empiète de ${Math.round(outside[k])} m² sur les retraits${where} : ajuste la forme, ou indique ce côté en rue / mitoyen dans le tableau des retraits` });
    });
