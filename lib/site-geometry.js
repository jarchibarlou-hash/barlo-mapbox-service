  ...
  function scenarioActual(units, ctx) {
    ...
    list.forEach((u, k) => {
      if (outside[k] >= 0.5) checks.push({ code: "HORS_ZONE_CONSTRUCTIBLE", level: "error", unit: u.name, m2: Math.round(outside[k]),
        message: `« ${u.name} » dépasse la zone constructible de ${Math.round(outside[k])} m² (retraits) : ajuste la forme, ou indique les côtés rue / mitoyen dans le tableau des retraits` });
    });
