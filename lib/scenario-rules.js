...
// - A, B et C gardent le MÊME PROGRAMME (nombre et types d'unités). B et C optimisent les surfaces
//   par typologie (grille validée par Jeremy le 23/09, pratique pro : les petits logements ont moins
//   de marge, plancher absolu par typologie, commerce inchangé car c'est la surface louée) ; le prix
//   suit (surface × coût/m² du rôle). Le budget est COMPARÉ (réserve comprise), jamais imposé en
//   supprimant des unités.
// - B = plans optimisés, réserve pour imprévus 10 % dans la comparaison au budget.
// - C = plans compacts, réserve 20 %, 85 % du COS, un niveau de moins en phase 1, volume compact ;
//   si C dépasse le budget, ce qui ne tient pas est REPORTÉ EN PHASE 2 (typologies gardées).
//   Pas de marge de retrait ajoutée (pénalisait les petites parcelles).
// Les valeurs numériques restent des valeurs par défaut BARLO (modifiables par lead), jamais des faits.
(function (root, factory) {
  ...
  // adapt_program     : le programme peut être phasé pour respecter les limites (C seulement)
  // adapt_mode        : PHASE_2 (report en phase 2) | DOWNGRADE (typologie d'un cran, puis retrait :
  //                     plus utilisé par défaut, conservé pour un réglage par lead)
  // surface_small     : réduction des surfaces T1/T2 (0,05 = −5 %)
  // surface_large     : réduction des surfaces T3/T4/T5
  ...
  // budget_target     : part du budget utilisable, le reste = réserve pour imprévus (null = pas de réserve)
  // compact           : volume unique compact (pas de programme scindé en deux volumes)
  const DEFAULT_RULES = Object.freeze({
    CLIENT_INTENT: Object.freeze({ adapt_program: false, adapt_mode: null,      surface_small: 0,    surface_large: 0,    emprise_usage_max: 1,    setback_extra_m: 0, levels_cap_delta: 0,  budget_target: null, compact: false }),
    BALANCED:      Object.freeze({ adapt_program: false, adapt_mode: null,      surface_small: 0.05, surface_large: 0.10, emprise_usage_max: 1,    setback_extra_m: 0, levels_cap_delta: 0,  budget_target: 0.9,  compact: false }),
    PRUDENT:       Object.freeze({ adapt_program: true,  adapt_mode: "PHASE_2", surface_small: 0.10, surface_large: 0.20, emprise_usage_max: 0.85, setback_extra_m: 0, levels_cap_delta: -1, budget_target: 0.8,  compact: true }),
  });

  // Surfaces minimales absolues par typologie (validées par Jeremy le 23/09) : jamais en dessous.
  const SURFACE_MIN_M2 = Object.freeze({ T1: 25, T2: 38, T3: 50, T4: 62, T5: 75 });
  const SMALL_TYPES = ["T1", "T2"];
  // Surface d'une unité pour un rôle : grille du standing réduite selon la typologie, plancher absolu.
  // Commerce et types inconnus : inchangés.
  function unitSurface(type, baseSize, rules) {
    const base = Number(baseSize) || 0;
    if (!(type in SURFACE_MIN_M2) || !rules) return base;
    const pct = Number(SMALL_TYPES.includes(type) ? rules.surface_small : rules.surface_large) || 0;
    const reduced = Math.round(base * (1 - pct));
    return Math.max(Math.min(base, SURFACE_MIN_M2[type]), reduced);
  }
  const RULES_SOURCE = "HYPOTHESIS";
  const RULES_NOTE = "Valeurs par défaut BARLO (hypothèses), modifiables par lead";

  // ── Budget du client : fourchette complète (bas ET haut), jamais une seule borne ──
  // Formats du questionnaire : "⭕ 50 000 – 100 000 € (~33–66 M FCFA)", "40 000 - 50 000 €",
  // "500 000 € – 1 M €", "328–656 M FCFA", "30 000 000 FCFA", ou un nombre.
  const EUR_TO_FCFA = 655.957; // parité fixe du franc CFA
  function parseAmounts(text) {
    const out = [];
    const re = /(\d{1,3}(?:[\s .]\d{3})+|\d+(?:[.,]\d+)?)\s*(M|k)?(?![\d])/gi;
    let m;
    while ((m = re.exec(text))) {
      const raw = m[1];
      const grouped = /[\s .]\d{3}$/.test(raw) && !/[.,]\d{1,2}$/.test(raw);
      let n = grouped ? Number(raw.replace(/[\s .]/g, "")) : Number(raw.replace(",", "."));
  ...
  function parseBudgetRange(raw) {
    if (typeof raw === "number") {
      ...
      const v = raw >= 1e6 ? raw : raw * EUR_TO_FCFA; // < 1 M : euros, sinon déjà en FCFA
      return { min: Math.round(v), max: Math.round(v), source: "QUESTIONNAIRE" };
    }
    const s = String(raw || "");
    if (!s.trim()) return null;
    // Montants en FCFA donnés par le formulaire : prioritaires (déjà convertis)
    const fcfaPart = s.match(/([\d\s .,–-]+(?:M)?)\s*FCFA/i);
    let values = [];
    if (fcfaPart) {
      const seg = fcfaPart[1];
      const hasM = /M\s*$/i.test(seg.trim()) || /M\s*FCFA/i.test(fcfaPart[0]);
      values = parseAmounts(seg.replace(/M\s*$/i, "")).map(v => (hasM && v < 1e6 ? v * 1e6 : v));
    } else {
      values = parseAmounts(s.replace(/\(.*?\)/g, " ")).filter(v => v >= 1000).map(v => v * EUR_TO_FCFA);
    }
    values = values.filter(v => v > 0);
    if (!values.length) {
      const n = Number(s.replace(/[\s ]/g, ""));
      return n > 0 ? parseBudgetRange(n) : null;
    }
    // « Plus de 1 M € » : fourchette ouverte vers le haut (le haut est inconnu, jamais inventé)
    const openEnded = /plus de|au[- ]del[aà]|sup[ée]rieur|>\s*\d/i.test(s);
    return { min: Math.round(Math.min(...values)), max: Math.round(Math.max(...values)), open_ended: openEnded, source: "QUESTIONNAIRE" };
  }
  // Position d'un besoin de financement dans la fourchette du client :
  // DANS_BUDGET (≤ bas de fourchette), BUDGET_TENDU (dans la fourchette, vers le haut),
  // HORS_BUDGET (au-dessus du haut ; jamais pour une fourchette ouverte)
  function budgetStatus(needed, range) {
    if (!range || !(range.max > 0) || !(needed > 0)) return "N/A";
    if (needed <= range.min) return "DANS_BUDGET";
    if (needed <= range.max || range.open_ended) return "BUDGET_TENDU";
    return "HORS_BUDGET";
  }
  ...
  function scenarioSdpLimits(o) {
    ...
    const limits = {
      ...
      // Le budget ne dimensionne que les rôles qui phasent (C) ; ailleurs il est comparé, pas imposé
      budget: (budgetKnown && r.adapt_program) ? (o.budget_fcfa * r.budget_target) / (o.cost_per_m2 * 1.05) : Infinity,
    };
    ...
    return {
      ...
      reserve_pct: r.budget_target ? Math.round((1 - r.budget_target) * 100) : null,
    };
  }
  ...
  return {
    ...
    DEFAULT_RULES, RULES_SOURCE, RULES_NOTE, SURFACE_MIN_M2, unitSurface, scenarioSdpLimits, fitProgram,
    EUR_TO_FCFA, parseBudgetRange, budgetStatus,
  };
});
